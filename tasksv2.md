...
- [ ] Add TermAI prompt override file for minimal-fork launch:
  - [ ] Add `docs/termai-system.md` (example TermAI system prompt to use with
        `GEMINI_SYSTEM_MD`).
  - [ ] Update `README.md` with a dedicated “TermAI prompt override” section
        showing `GEMINI_SYSTEM_MD=... gemini`.
  - [ ] Add a short pointer in `docs/get-started/index.md` (or most appropriate
        get-started page) to `GEMINI_SYSTEM_MD` usage.
- [ ] Add regression test: “enabling CodebaseInvestigator does not reintroduce
...
- [ ] Add/extend unit tests for `WebSearchTool` citation insertion correctness:
...
- [ ] Add/extend unit tests for `WebFetchTool` URL parsing and safety:
...
- [ ] Add a TermAI “online info” recipe section (docs-only, no code) covering
      safe patterns:
...
- [ ] Add a “best practice” doc section for searching large repos efficiently:
...
- [ ] Add `FileOpsTool` (workspace-scoped, safe defaults):
  - [ ] Create `packages/core/src/tools/file-ops.ts`
  - [ ] Add tool name constant in `packages/core/src/tools/tool-names.ts` (+
        `packages/core/src/tools/tool-names.test.ts`)
  - [ ] Register in `packages/core/src/config/config.ts` (inside
        `createToolRegistry()` via existing `registerCoreTool(...)`)
  - [ ] Add unit tests: `packages/core/src/tools/file-ops.test.ts`
  - Operations (explicit + typed, bounded output):
    ...
    - `listTree`: `{ path?, maxDepth?, maxEntries? }` (bounded, for “organize my
      folder” workflows)
  - Safety rules:
...
- [ ] Add a TermAI “Operator Recipes” page with concrete prompts and expected
      behavior:
  ...
  - Include a “safe pattern” section: preview → explain → confirm → execute →
    recap.
- [ ] Add a small set of example custom commands (docs-only) that wrap common
      recipes:
...
- [ ] Document the manual verification recipe and expected outcomes:
...
- [ ] Ensure agent-control cannot silently bypass workspace trust boundaries or
